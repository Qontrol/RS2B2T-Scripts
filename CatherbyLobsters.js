...
class CatherbyLobsters extends LoopingBot {
    status = 'starting';
    startedAt = 0;
    fishXpAtStart = 0;
    cookXpAtStart = 0;
    ...
    async onStart() {
        ...
        this.log(
            `CatherbyLobsters @ ${ANCHOR.x},${ANCHOR.z} — start always banks everything ` +
                `(deposit → unequip → deposit → withdraw Lobster pot); Cage on Cage+Harpoon; ` +
...
export default defineBot({
    ...
    description:
        "Benzyme's Catherby Lobsters — on start always banks, deposits everything, withdraws Lobster pot, then cages lobsters on Cage+Harpoon spots. Buys a pot from Harry if missing. Optional cook on way; optional sell Raw lobster to Harry instead of banking.",
    settingsSchema: {
        cookOnWay: {
            type: 'boolean',
            default: true,
